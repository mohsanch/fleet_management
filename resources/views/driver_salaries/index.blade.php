...
@section('title', 'Driver Salaries')
@section('breadcrumbs', 'Payroll / Driver Salaries')
@section('page_title', 'Driver Salary Payroll')
    ...
    <div class="chart-header" style="display: flex; flex-direction: column; align-items: flex-start; gap: 16px; width: 100%; border-bottom: none; padding-bottom: 0;">
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; flex-wrap: wrap; gap: 15px;">
            <div class="chart-title-block">
                <span class="chart-title">Driver Salary Disbursements</span>
                <span class="chart-subtitle">View gross pay, deductions, pasgi adjustments and net payable per driver</span>
            </div>
            @can('payroll.create')
            <a href="{{ route('driver-salaries.create') }}" class="btn-signin" style="padding: 10px 18px; width: auto; display: inline-flex; align-items: center; gap: 8px; font-size: 12px; margin: 0; white-space: nowrap;">
        ...
        <div style="width: 100%; display: flex; justify-content: flex-start; align-items: center; background: rgba(0,0,0,0.02); padding: 12px 16px; border-radius: 8px; border: 1px solid var(--border-color); box-shadow: inset 0 1px 2px rgba(0,0,0,0.02);">
            <form action="{{ route('driver-salaries.index') }}" method="GET" style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin: 0; width: 100%;">
                ...
                <input type="month" name="period" class="form-input" value="{{ request('period') }}" style="height: 38px; padding: 5px 15px; font-size: 13px; width: auto; margin: 0;" title="Salary Period">
                <div style="display: flex; align-items: center; gap: 5px;">
                    <span style="font-size: 11px; color: var(--text-muted); white-space: nowrap;">Paid From:</span>
                ...
                <div style="display: flex; align-items: center; gap: 5px;">
                    <span style="font-size: 11px; color: var(--text-muted); white-space: nowrap;">Paid To:</span>
                ...
                @if(request('search') || request('driver_id') || request('period') || request('date_from') || request('date_to'))
                    <a href="{{ route('driver-salaries.index') }}" class="btn-signin" style="padding: 10px 20px; height: 38px; margin: 0; width: auto; font-size: 12px; background: #718096; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; color: #fff;">Clear</a>
                @endif
            </form>
        </div>
    </div>
        ...
        <table class="purity-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <strong style="color: var(--text-color);">{{ $salary->driver->name }}</strong>
                    </td>
                    <td>
                        <span style="color: #38A169; font-weight: 600;">Rs. {{ number_format($salary->gross_salary) }}</span>
                    </td>
                    <td>
                        <span style="color: #E53E3E;">Rs. {{ number_format($salary->fine ?? 0) }}</span>
                    </td>
                    <td>
                        <span style="color: #E53E3E;">Rs. {{ number_format($salary->pasgi_adjustment ?? 0) }}</span>
                    </td>
                    <td>
                    ...
                    <td style="text-align: right;">
                        <div style="display: inline-flex; gap: 15px; align-items: center;">
                            @can('payroll.edit')
                                <a href="{{ route('driver-salaries.edit', $salary->id) }}" style="color: var(--primary); text-decoration: none; font-weight: 700; font-size: 12px;">Edit</a>
                            @endcan
                            @can('payroll.delete')
                                <form action="{{ route('driver-salaries.destroy', $salary->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
