...
@section('breadcrumbs', 'Payroll / Employee Advances / Edit')
...
<div class="card" style="max-width: 480px; margin: 0 auto;">
    <div class="chart-header" style="margin-bottom: 24px;">
        <div class="chart-title-block">
            <span class="chart-title">Edit Employee Advance</span>
            <span class="chart-subtitle">Modify this staff advance record</span>
        </div>
    </div>

    <form action="{{ route('employee-advances.update', $employeeAdvance->id) }}" method="POST" class="auth-form" style="max-width: 100%; gap: 20px;">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <input type="date" name="date" id="date" class="form-input" value="{{ old('date', $employeeAdvance->date) }}" required>
                @error('date') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="amount">Amount (Rs.)</label>
                <input type="number" step="0.01" name="amount" id="amount" class="form-input" value="{{ old('amount', $employeeAdvance->amount) }}" required>
        ...
        <div class="form-group">
            ...
            <textarea name="remarks" id="remarks" class="form-input" style="height: 80px; padding: 10px 20px; resize: none;">{{ old('remarks', $employeeAdvance->remarks) }}</textarea>
