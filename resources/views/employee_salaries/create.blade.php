...
@section('title', 'Process Employee Salary')
@section('breadcrumbs', 'Payroll / Employee Salaries / Process')
@section('page_title', 'Process Employee Salary')

@section('content')
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <div class="chart-header" style="margin-bottom: 24px;">
        <div class="chart-title-block">
            <span class="chart-title">New Employee Salary</span>
            <span class="chart-subtitle">Enter gross pay, fines, and advance adjustments</span>
        </div>
    </div>

    <form action="{{ route('employee-salaries.store') }}" method="POST" class="auth-form" style="max-width: 100%; gap: 20px;">
        @csrf
        ...
        <div style="border-top: 1px solid rgba(255,255,255,0.08); padding-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="gross_salary" id="gross_salary" class="form-input" placeholder="Auto-filled from employee profile" value="{{ old('gross_salary') }}" required oninput="calcNet()">
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="advance_adjustment" id="advance_adjustment" class="form-input" placeholder="0.00" value="{{ old('advance_adjustment', 0) }}" oninput="calcNet()">
                @error('advance_adjustment') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="other_adjustment">Other Adjustment +/- (Rs.)</label>
                <input type="number" step="0.01" name="other_adjustment" id="other_adjustment" class="form-input" placeholder="0.00" value="{{ old('other_adjustment', 0) }}" oninput="calcNet()">
        ...
        <div class="form-group">
            ...
            <textarea name="remarks" id="remarks" class="form-input" placeholder="Any notes about this salary..." style="height: 75px; padding: 10px 20px; resize: none;">{{ old('remarks') }}</textarea>
        </div>
